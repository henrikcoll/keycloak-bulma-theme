...
<@layout.mainLayout active='applications' bodyClass='applications'; section>
	...
	<h2 class="title is-2">${msg("applicationsHtmlTitle")}</h2>
                ...
                <tr>
                    ...
                    <td>
                        <#if (application.client.consentRequired && application.clientScopesGranted?has_content) || application.additionalGrants?has_content>
                            <button type='submit' class="button is-danger" id='revoke-${application.client.clientId}' name='clientId' value="${application.client.id}">${msg("revoke")}</button>
                        </#if>
                    </td>
                </tr>
              </#list>
